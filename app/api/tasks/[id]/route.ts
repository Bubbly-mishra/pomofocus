...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  await db.collection("tasks").deleteOne({ _id: new ObjectId(params.id), userId: session.userId })
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  await db.collection("tasks").updateOne({ _id, userId: session.userId }, { $set: update })
  const doc = await db.collection("tasks").findOne({ _id, userId: session.userId })
